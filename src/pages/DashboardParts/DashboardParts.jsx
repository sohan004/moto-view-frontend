...
const DashboardParts = () => {
    ...
    const allCategories = useGetCategory('parts');
    ...
    useEffect(() => {
        const fetchBrands = async () => {
            try {
                setLoading(true);
                const { data } = await axios.get(`/parts?type=${activeTab}`);
                if (data.length < 10) {
    ...
    const loadMore = async () => {
        try {
            const { data } = await axios.get(`/parts?type=${activeTab}&skip=${categories.length}`);
    ...
    const onSubmit = async () => {
        try {
            ...
            setLoading(true);
            setErr('');
            const { data } = await axios.post('/parts', formData, {
                headers: {
    ...
    const changeStatus = async (id, currentStatus) => {
        try {
            await axios.put(`/parts/change-status/${id}`, { status: !currentStatus });
        ...
        <div>
            <div className='grid grid-cols-2  bg-white '>
                <p
                    ...
                    className={`font-semibold text-center hover:border-red-500 border-b-2 py-3 text-sm cursor-pointer md:text-base capitalize ${getActiveTab('bike')}`}>bike parts</p>
                <p
                    onClick={() => setActiveTab('car')}
                    className={`font-semibold text-center hover:border-red-500 border-b-2 py-3 text-sm cursor-pointer md:text-base capitalize ${getActiveTab('car')}`}>car parts</p>
            </div>
            ...
            <div>
                <button
                    ...
                    className="btn btn-success text-white mt-4 btn-sm">create new parts</button>
            </div>
            ...
            <dialog id="accessories-modal" className="modal">
                <div className="modal-box bg-white  relative select-none ">
                    ...
                    <div className='grid gap-5'>
                        <div>
                            <p className='font-medium mb-1'>select parts image*</p>
                            <input
